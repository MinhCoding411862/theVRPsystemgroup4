...
public class MasterAgent extends Agent {
            ...
            if (isSpecific) {
                ...
                if (selectedInfo != null) {
                    ...
                    if (!fitsWeight || !fitsItemCount) {
                        selectedInfo = null; // Không vừa, từ chối
                    }
                }
            } else {
    ...
    private PackageInfo findBestPackage(int currentLoad, int capacity, int currentItems, int maxItems) {
        for (PackageInfo pkg : availablePackages.values()) {
            if ((currentLoad + pkg.weight <= capacity) && (currentItems + 1 <= maxItems)) {
                return pkg;
            }
        }
    ...
    private void handlePackagePickup(ACLMessage msg) {
        ...
        synchronized (availablePackages) {
            ...
            if (pkg != null) {
                ...
                gui.addMessage("⚠️ ERROR: Package " + packageName + " not in offered list!");
            }
        }
    }
    ...
    private void pauseSystem() {
        ...
        gui.addMessage("");
        gui.addMessage("========================================");
        gui.addMessage("⏸️  SYSTEM PAUSED");
        gui.addMessage("========================================");
    ...
    private void resumeSystem() {
        ...
        gui.addMessage("");
        gui.addMessage("========================================");
        gui.addMessage("▶️  SYSTEM RESUMED");
        gui.addMessage("========================================");
