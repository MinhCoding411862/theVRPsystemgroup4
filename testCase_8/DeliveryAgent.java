...
public class DeliveryAgent extends Agent {
    ...
    private void handlePackageOffer(ACLMessage msg) {
        ...
        boolean isPriority = p.length > 3 && p[3].equals("PRIORITY");

        boolean fitsWeight = (currentLoad + weight <= capacity);
        boolean fitsItems = (currentItems + 1 <= maxItems);

        if (isPriority) {
            SwingUtilities.invokeLater(() -> {
                MasterAgent.gui.addMessage("Received PRIORITY offer: " + pkgName + " -> Must negotiate");            });
        }

        if (isPriority || !fitsWeight || !fitsItems) {
        ...
        currentLoad += weight;
        currentItems++;
        packagesCarrying.add(new PackageInTransit(pkgName, baseTime, weight, speedFactor, isPriority));
        ...
        if (currentLoad < capacity && currentItems < maxItems) {
            ...
            SwingUtilities.invokeLater(() -> {
                String reason = (currentItems >= maxItems) ? "Item limit" : "Weight limit";
                MasterAgent.gui.addMessage("FULL -> " + getLocalName() + " (" + reason + ") | Starting delivery");            });
            startDelivery();
        }
    }
    ...
    private void evaluateProposals(String packageName) {
        ...
        if (receivedProposals.isEmpty()) {
            SwingUtilities.invokeLater(() -> {
                MasterAgent.gui.addMessage("   NO HELP -> " + getLocalName() +
    ...
    private void startDelivery() {
        isAtMaster = false;
        packagesCarrying.sort(
                Comparator.comparing((PackageInTransit p) -> p.priority ? 0 : 1)
                        .thenComparingInt(p -> p.actualTravelTime)
        );
    ...
    static class PackageInTransit {
        ...
        int weight;
        boolean priority;  // <--- THÃŠM

        PackageInTransit(String n, int b, int w, double f, boolean p) {
            name = n; baseTravelTime = b; weight = w; priority = p;
