...
public class DeliveryGUI extends JFrame {
    ...
    private Runnable onPriorityCallback;  // <--- THÃŠM
    ...
    public DeliveryGUI() {
        ...
        // Control panel
        JPanel controlPanel = new JPanel(new GridLayout(4, 1, 5, 5));  // <--- 4 dÃ²ng
        ...
        // <--- NÃšT Táº O PRIORITY PACKAGE ---
        JButton priorityBtn = new JButton("Create Priority Package");
        priorityBtn.setFont(new Font("Arial", Font.BOLD, 12));
        priorityBtn.setBackground(new Color(255, 100, 100));
        priorityBtn.setForeground(Color.WHITE);
        priorityBtn.setFocusPainted(false);
        priorityBtn.addActionListener(e -> {
            if (onPriorityCallback != null) {
                onPriorityCallback.run();
            }
        });
        ...
        controlPanel.add(priorityBtn);  // <--- THÃŠM VÃ€O
        ...
        optimizationLabel = new JLabel("PRIORITY: Items: 0 | Distance: 0s | Efficiency: 0.000 items/s");
    ...
    public void addMasterPackage(String packageName, int travelTime, int weight, boolean priority) {
        SwingUtilities.invokeLater(() -> {
            ...
            pkgPanel.setBackground(priority ? new Color(255, 220, 220) : new Color(240, 245, 255));

            JLabel boxIcon = new JLabel(priority ? "Priority Package" : "Package");
    ...
    public void addAgent(String agentName, int capacity) {
        ...
        agentPanel.setBorder(BorderFactory.createTitledBorder("ðŸšš " + agentName));
    ...
    public void setOnPriorityCallback(Runnable callback) {
    ...
    public void updateOptimizationStats(int totalItems, int totalDistance) {
        SwingUtilities.invokeLater(() -> {
            ...
            optimizationLabel.setText(String.format(
                    "ðŸŽ¯ PRIORITY: Items: %d | Distance: %ds | Efficiency: %.3f items/s",
